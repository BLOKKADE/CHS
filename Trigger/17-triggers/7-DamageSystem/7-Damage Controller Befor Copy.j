...
endglobals
 ...
 function SetTypeDamage takes unit u returns nothing
    ...
    if GetEventDamage() < -890000 then
        ...
        set GLOB_ABIL_ID = IdDamage.id
        
        
        
        
        
        if IdDamage.number == 1 then
            call BlzSetEventDamage(GetAbilityData(u,GLOB_ABIL_ID).GetDamage1())
        elseif IdDamage.number == 2 then
            call BlzSetEventDamage(GetAbilityData(u,GLOB_ABIL_ID).GetDamage2())
        elseif IdDamage.number == 3 then
            call BlzSetEventDamage(GetAbilityData(u,GLOB_ABIL_ID).GetDamage3())
        endif
