library Dreadlord requires HideEffects
    function DreadlordLifeSteal takes unit source, unit target returns nothing
        call SetUnitState(source, UNIT_STATE_LIFE, GetUnitState(source, UNIT_STATE_LIFE) + (GetUnitState(source, UNIT_STATE_MAX_LIFE) * 0.1))
