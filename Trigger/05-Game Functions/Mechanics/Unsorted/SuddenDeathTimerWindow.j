library SuddenDeathTimerWindow requires TimerUtils, Utility
    ...
    function UpdateSuddenDeathTimer takes nothing returns nothing
        ...
        if level == 2 then
            call PlayerText("|cffffee00Sudden Death Level 1|r: Midas Touch and Summon Upgrades disabled")
            set duration = 15
        elseif level == 3 then
            call PlayerText("|cffff9900Sudden Death Level 2|r: Creeps get attack damage bonus and maximum movespeed")
            set duration = 15
        elseif level == 4 then
            call PlayerText("|cffff6600Sudden Death Level 3|r: 20% current hit point drain every 0.25 seconds.")
            return
        endif
