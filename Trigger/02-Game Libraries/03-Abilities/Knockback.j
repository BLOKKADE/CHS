...
library Knockback initializer Init needs TerrainPathability, GroupUtils, Table, UnitItems, TimerUtils
    ...
    function KnockbackTarget takes unit source, unit targ, real angle, real startspeed, real decrement, boolean killDestructables, boolean knockAdjacent, boolean chainAdjacent, boolean override returns boolean
        ...
        if override == false then
            if BlzIsUnitInvulnerable(targ) or GetUnitAbilityLevel(targ, ENTANGLING_ROOTS_BUFF_ID) > 0 or GetUnitAbilityLevel(targ, HARDENED_SKIN_ABILITY_ID) > 0 or UnitHasItemType(targ, 'I090') or UnitHasItemType(targ, 'I0CV') or (GetUnitTypeId(targ) == STOMP_TREE_UNIT_ID and GetHeroLevel(targ) > 150) then
            return false
            endif
        endif
