library LightRune requires RandomShit
    ...
    function CastLightRune takes nothing returns boolean
        if IsUnitEnemy(GLOB_RUNE_U,GetOwningPlayer(GetFilterUnit())) and IsUnitTarget(GetFilterUnit()) then
            ...
            call UnitDamageTarget(GLOB_RUNE_U, GetFilterUnit(), (2*GLOB_RUNE_POWER) * BlzGetUnitMaxHP(GLOB_RUNE_U), false, false, ATTACK_TYPE_NORMAL,DAMAGE_TYPE_MAGIC,WEAPON_TYPE_WHOKNOWS)
        endif
        return false
    endfunction
