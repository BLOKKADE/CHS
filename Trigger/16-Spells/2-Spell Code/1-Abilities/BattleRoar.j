library BattleRoar requires RuneInit
    function CastBattleRoar takes unit caster, integer abilId, integer lvl returns nothing
        ...
        call EnumTargettableUnitsInRange(ENUM_GROUP, GetUnitX(caster), GetUnitY(caster), 650, GetOwningPlayer(caster), false)

        loop
            set p = FirstOfGroup(ENUM_GROUP)
            exitwhen p == null

            if IsUnitAlly(p, GetOwningPlayer(caster)) then
