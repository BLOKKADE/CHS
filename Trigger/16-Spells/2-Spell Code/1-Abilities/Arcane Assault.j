    ...
    function ArcaneAssault takes unit damageSource, unit damageTarget, real damage returns nothing
        ...
        loop
            ...
            if targets > 0 then
                ...
                call UnitDamageTarget(damageSource, target, damage, true, IsUnitType(damageSource, UNIT_TYPE_RANGED_ATTACKER), ATTACK_TYPE_NORMAL, DAMAGE_TYPE_NORMAL, null)
                //call BJDebugMsg("full aa" + I2S(GetHandleId(target)) + " targets: " + I2S(targets))
            elseif damageBonus != 0 then
                set GLOB_typeDmg = 2
                set DamageIsAttack = true
                call UnitDamageTarget(damageSource, target, damage * damageBonus, true, IsUnitType(damageSource, UNIT_TYPE_RANGED_ATTACKER), ATTACK_TYPE_NORMAL, DAMAGE_TYPE_NORMAL, null)
