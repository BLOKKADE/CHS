library AncientTeaching initializer init requires Cooldown, StableSpells, ToggleSpell
    ...
    private function ResetAbility_A05U takes unit u,integer abilId returns nothing
        if GetUnitAbilityLevel(u, ANCIENT_TEACHING_ABILITY_ID) > 0 and BlzGetUnitAbilityCooldownRemaining(u, ANCIENT_TEACHING_ABILITY_ID) <= 0.001 and IsSpellResettable(abilId) and IsSpellResettable(GetOriginalSpell(u, abilId)) then
