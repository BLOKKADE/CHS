library Overload initializer init requires RandomShit, AbilityData, DummyOrder, CastSpellOnTarget

function Overload takes unit caster, unit target, integer abilId, integer lvl returns nothing
    local integer i = 0
    local unit p = null
    ...
    local real baseMana = BlzGetAbilityManaCost(abilId, lvl - 1)
    local real mana = baseMana * manaFactor
